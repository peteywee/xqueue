...
test('explicit rebind versions the assignment and preserves superseded history',()=>{
  ...
  const p=planAssignmentRebind({content:s.content,activeAssignment:s.activeAssignment,targetRevision:target,publicationState:s.publicationState,reason:'activate correction',now:NOW});
...
test('owner cancellation preserves evidence and aligns publication_state skip',()=>{
  ...
  const p=planAssignmentCancel({content:s.content,activeAssignment:s.activeAssignment,publicationState:s.publicationState,reason:'owner withdrew future post',now:NOW});
...
test('publication-state CAS race causes zero owner-state mutation and zero false evidence',()=>{
  for(const kind of ['revise','rebind','cancel']){
    ...
    if(kind==='revise'){
      ...
      p=planAssignmentRebind({content:s.content,activeAssignment:s.activeAssignment,targetRevision:s.revision,publicationState:s.publicationState,reason:'race',now:NOW});
      sql=renderRebindSql(p,{recordedAt:NOW});
    } else {
      p=planAssignmentCancel({content:s.content,activeAssignment:s.activeAssignment,publicationState:s.publicationState,reason:'race',now:NOW});
...
test('stale competing cancel cannot overwrite the winning reason or append evidence',()=>{
  const db=fixture(), s=snap(db);
  const win=planAssignmentCancel({content:s.content,activeAssignment:s.activeAssignment,publicationState:s.publicationState,reason:'winner',now:NOW});
  const stale=planAssignmentCancel({content:s.content,activeAssignment:s.activeAssignment,publicationState:s.publicationState,reason:'stale',now:NOW});
...
test('media-bound rebind requires exact media and rechecks readiness at mutation time',()=>{
  ...
  const s=snap(db);
  assert.throws(()=>planAssignmentRebind({content:s.content,activeAssignment:s.activeAssignment,targetRevision:target,publicationState:s.publicationState,reason:'media',now:NOW}),/media is required/);
  const p=planAssignmentRebind({content:s.content,activeAssignment:s.activeAssignment,targetRevision:target,targetMedia:media,publicationState:s.publicationState,reason:'media',now:NOW});
