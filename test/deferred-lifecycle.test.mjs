...
import { isResolved } from '../src/runtime-health.mjs';
import {
  nextRuntimeRevision,
...
test('repeated scheduler wakes do not create a catch-up publication burst', () => {
  const queue = [
    {
      id: 'P1',
      scheduledAt: '2026-09-21T12:00:00.000Z',
    ...
    {
      id: 'P2',
      scheduledAt: '2026-09-21T12:05:00.000Z',
      scheduledDate: '2026-09-21',
      scheduledTime: '07:05',
      timezone: 'America/Chicago',
      slot: 'slot-2',
  ...
  const first = deferMissedStaticAssignments(queue, state, {
    now,
    graceMinutes: 20,
    policyVersion: 2,
  });
  const second = deferMissedStaticAssignments(queue, state, {
    now: new Date('2026-09-21T13:15:00.000Z'),
    graceMinutes: 20,
    policyVersion: 2,
  });
  ...
  assert.deepEqual(Object.keys(state.skipped), []);
  assert.equal(isResolved(state, 'P1'), true);
  assert.equal(isResolved(state, 'P2'), true);
});

test('confirmed rate-limit and media results map to deferred handoff, ambiguity does not', () => {
  assert.equal(
    publicationDeferralHandoff({
      status: 'confirmed_not_posted',
      evidence: {
        outcome: {
...
test('durable missed deferral promotes a new runtime revision and remains fail-closed/idempotent', async () => {
  const db = new DatabaseSync(':memory:');
  for (const name of [
  ...
  const initialRevision = nextRuntimeRevision({
    currentState: null,
    snapshot: initialSnapshot,
    sourceOperationId: 'fixture-bootstrap',
    recordedAt: at,
  });
  db.exec(renderRuntimeRevisionInsertSql(initialRevision));

  const first = await deferMissedAssignments(api, {
    now: new Date('2026-09-21T12:20:00.001Z'),
    graceMinutes: 20,
  });

  assert.equal(first.outcomes.length, 1);
  assert.equal(first.outcomes[0].status, 'deferred');
  assert.equal(first.runtimeRevision.status, 'promoted');
  assert.equal(first.runtimeRevision.generation, 2);

  const latest = db.prepare(
    'SELECT * FROM queue_runtime_revisions ORDER BY generation DESC LIMIT 1',
  ).get();
  assert.equal(latest.generation, 2);
  assert.equal(latest.active_assignment_count, 0);
  assert.equal(latest.previous_revision_digest, initialRevision.revision_digest);

  const verified = await verifyDynamicRuntime(
    { DB: api },
    { verifyMedia: false },
  );
  assert.equal(verified.ok, true);
  assert.equal(verified.generation, 2);
  assert.equal(verified.activeAssignmentCount, 0);
  assert.equal(verified.deferredCount, 1);

  const replay = await deferMissedAssignments(api, {
    now: new Date('2026-09-21T12:40:00.000Z'),
    graceMinutes: 20,
  });
  assert.equal(replay.outcomes.length, 0);
  assert.equal(replay.runtimeRevision.status, 'unchanged');
...
test('confirmed handoff defers within grace and advances runtime revision', async () => {
  ...
  const api = d1Adapter(db);
  const initialSnapshot = await buildDynamicRuntimeSnapshot(
    await readDynamicRuntimeRows(api),
  );
  ...
  const row = baseRow({ content_digest: digest });
  const result = await deferConfirmedPublicationHandoff(
    api,
    row,
    {
      status: 'confirmed_not_posted',
  ...
  assert.equal(result.status, 'deferred');
  assert.equal(result.reason, 'confirmed_rate_limit_handoff');
  assert.equal(result.runtimeRevision.status, 'promoted');
  assert.equal(result.runtimeRevision.generation, 2);
  assert.equal(
    db.prepare("SELECT reason FROM queue_deferrals WHERE content_id='P1'").get().reason,
    'confirmed_rate_limit_handoff',
  );
});
